...
def main():
    ...
    log_info(f"{args}")
    all_metrics = main_eval_volume_retrieval(args)
    log_info("#################### volume retrieval ####################")
    for k in (5, 10, 50):
        log_info(f"MAP@{k:>3}: {all_metrics[f'vol_map{k}'] * 100:6.2f}")
    for k, v in all_metrics.items():
        if not k.startswith("vol_map"):
            log_info(f"{k:>20}: {v:6.2f}" if isinstance(v, float) else f"{k:>20}: {v}")


def main_eval_volume_retrieval(args: Args):
    device = "cuda"

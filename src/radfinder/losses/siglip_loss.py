...
class SigLIPLoss(nn.Module):
    ...
    def forward(
        ...
        if not dist.is_initialized():
            ...
            pos_ll, neg_ll = self._slice_loglik(logits, include_pos=True)
        ...
        pos_sum = torch.tensor(0.0, device=zimg.device, dtype=zimg.dtype)
        neg_sum = torch.tensor(0.0, device=zimg.device, dtype=zimg.dtype)
        ...
        for k in range(world_size):
            ...
            if k == rank:
                ...
                pos_ll, neg_ll = self._slice_loglik(logits, include_pos=False)

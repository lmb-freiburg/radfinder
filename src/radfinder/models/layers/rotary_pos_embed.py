...
class RotaryPositionEmbedding(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # min_period: float | None = None,  # unused
        # max_period: float | None = None,  # unused
        normalize_coords: Literal["min", "max", "separate"] = "separate",  # same
    ...
    def _init_weights(self):
        ...
        if self.base is not None:
            ...
            raise NotImplementedError("Disabled non-'base' for simplicity")
    ...
    def forward(
        ...
        *,
        ...
        """
        Args:
            H, W, D: grid dimensions (number of tokens per axis)
            H_div, W_div, D_div: optional to override normalization
                denominators. When None, defaults to (H, W, D). Use this to preserve
                pretrained inter-token spacing when the grid resolution changes, e.g.
                H_div, W_div, D_div=(3, 3, 4) with D=32 gives the same spacing as pretrained D=4.
        Returns:
        ...
        assert self.normalize_coords == "separate", "Disabled non-'separate' for simplicity"
        ...
        H_div = H_div if H_div is not None else H
        W_div = W_div if W_div is not None else W
        D_div = D_div if D_div is not None else D
        coords_h = torch.arange(0.5, H, **dd) / H_div
        coords_w = torch.arange(0.5, W, **dd) / W_div
        coords_d = torch.arange(0.5, D, **dd) / D_div

        coords = torch.stack(
            torch.meshgrid(coords_h, coords_w, coords_d, indexing="ij"), dim=-1
        )  # [H, W, D, 3]

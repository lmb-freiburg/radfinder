...
Source: python3.12/site-packages/torch/nn/utils/weight_norm.py torch==2.9.1

`torch.nn.utils.weight_norm` is deprecated in favor of `torch.nn.utils.parametrizations.weight_norm`
However the new function is not compatible with the following code line in the model:
    self.last_layer.weight_g.data.fill_(1)  # type: ignore
So for now the model will use the old weight_norm version and this is copy of the old function,
a) in case torch decides to remove it next release, and b) to remove the warning.
"""

...
def torch_cat_ignore_nones(
    tensors: List[torch.Tensor | None],
    dim: int = 0,
    *,
    out=None,
    allow_all_none: bool = False,
) -> torch.Tensor:
    content = [t for t in tensors if t is not None]
    if len(content) == 0:
        if allow_all_none:
            return None
        raise ValueError("All tensors are None")
    return torch.cat(content, dim=dim, out=out)
...
def masked_average(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        (B, F)
    """
    assert x.ndim == 3 and mask.ndim == 2, f"{x.shape=}, {mask.shape=}"
    masku = mask.unsqueeze(2)
    return (x * masku).sum(dim=1) / masku.sum(dim=1).clamp(min=1)
...
def update_drop_path_rate(
    ...
    if mode == "linear":
        ...
        raise ValueError(f"Unknown mode: '{mode}', supported modes are 'linear' and 'uniform'.")
...
def set_at_index(tokens: torch.Tensor, index: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
    """
...
def patchify(images: torch.Tensor, patch_size: Tuple[int, int, int]) -> torch.Tensor:
    ...
    assert (
        H % patch_size[0] == 0 and W % patch_size[1] == 0 and D % patch_size[2] == 0
...
def resample_patch_embed(
    ...
    def resize(x_np, _new_size):
        ...
        x_upsampled = F.interpolate(x_tf, size=_new_size, mode=interpolation)[0, 0, ...].numpy()
        return x_upsampled
...
def uncat_with_shapes(
    x_cat: torch.Tensor, shapes: List[Tuple[int, ...]], num_tokens: List[int]
) -> List[torch.Tensor]:
    if not shapes:
        return []
